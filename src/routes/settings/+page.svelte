<script lang="ts">
	...
	const provider = $derived(authStore.user?.app_metadata?.provider ?? 'unknown');
...
{#if authStore.isAuthenticated}
	...
	<div class="flex max-w-lg flex-col gap-[var(--space-8)]">
		<!-- Profile section -->
		...
		<div
			class="flex flex-col gap-[var(--space-3)] border-t border-[var(--color-border)] pt-[var(--space-4)]"
		>
			<h2
				class="font-medium tracking-[0.05em] text-[var(--color-text-muted)] text-[var(--text-body)] uppercase"
			>
			...
			<div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-[var(--text-body)]">
				...
				<span class="font-mono text-[var(--color-text-primary)] text-[var(--text-aux)]">
			...
			{:else}
				<div class="flex flex-col gap-[var(--space-3)]">
					{#each passkeys as passkey (passkey.id)}
						<div
							class="flex flex-col gap-[var(--space-2)] border border-[var(--color-border)] p-[var(--space-3)]"
						>
							{#if renamingId === passkey.id}
								<div class="flex items-center gap-[var(--space-2)]">
									<input
										...
										class="h-[var(--height-control-sm)] flex-1 border border-[var(--color-border)] bg-transparent px-[var(--space-3)] text-[var(--color-text-primary)] text-[var(--text-body)] outline-none placeholder:text-[var(--color-text-muted)]"
									/>
							...
							{:else}
								<div class="flex items-center justify-between">
									<div class="flex flex-col gap-[var(--space-1)]">
										...
										>
											<span
												>{t.auth.passkeyCreated}: {passkey.created_at
													? new Date(passkey.created_at).toLocaleDateString()
													: '-'}</span
											>
											<span
												>{t.auth.passkeyLastUsed}: {passkey.last_used_at
